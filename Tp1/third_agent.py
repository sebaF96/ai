from tools import get_random_floor, get_starting_position, is_dirty, clear, log, get_direction

def start():
    floor = get_random_floor()
    position = get_starting_position(len(floor))
    direction = get_direction(position, len(floor))
    last_movement = f"Move {direction}"
    movements = 0
    cleaned_pos = set()
